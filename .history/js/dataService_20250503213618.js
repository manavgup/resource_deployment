...
const dataService = {
    ...
    loadExcelFile: function(file) {
        console.log(`[DataService] Starting to load Excel file: ${file.name}`);
        
        // Make sure we return the Promise
        return excelParser.parseExcelFile(file)
            .then(data => {
                this.rawData = data;
                console.log(`[DataService] Loaded ${this.rawData.length} records from Excel file`);
                
                // Process the data
                this.processData();
                console.log(`[DataService] Processed data into ${Object.keys(this.accountCounts).length} accounts`);
                
                // Extract filter options
                this.extractFilterOptions();
                console.log(`[DataService] Extracted filter options: ${this.filterOptions.brands.length} brands, ${this.filterOptions.accounts.length} accounts`);
            ...
            .catch(error => {
                // Log error
                console.error(`[DataService] Error loading Excel file:`, error);
