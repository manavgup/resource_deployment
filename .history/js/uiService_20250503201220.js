...
const uiService = {
     ...
     */
    // In uiService.js, update the handleFileSelected function

handleFileSelected: function(file) {
    const fileStatus = document.getElementById('fileStatus');
    
    // Check if fileStatus element exists before trying to use it
    if (!fileStatus) {
        console.log("File status element not found. Continuing with file processing...");
...
// Add a new method that doesn't depend on fileStatus
loadFileWithoutStatus: function(file) {
    app.loadFile(file)
        .then(() => {
            // Update fileStatus if it exists
            const fileStatus = document.getElementById('fileStatus');
            if (fileStatus) {
