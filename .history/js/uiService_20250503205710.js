...
const uiService = {
    ...
    handleFileSelected: function(file) {
        ...
        if (!uploadStatus) {
            console.log("Upload status element not found. Continuing with file processing...");
    ...
    // Add a new method that doesn't depend on uploadStatus
    loadFileWithoutStatus: function(file) {
        app.loadFile(file)
            .then(() => {
                ...
                this.logDebug(`File loaded successfully: ${file.name}`);
            })
            .catch(error => {
                // Update uploadStatus if it exists
                const uploadStatus = document.getElementById('uploadStatus');
                ...
                this.logDebug(`Error loading file: ${error.message}`);
